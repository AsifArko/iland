# Docker Setup for Fermi

This document provides comprehensive instructions for containerizing and running the Fermi Learning Management System using Docker.
...
Before using Docker with Fermi, ensure you have:
...
docker-compose -f docker-compose.dev.yml exec fermi-dev npm run lint

# Access container shell
docker-compose -f docker-compose.dev.yml exec fermi-dev sh

# Restart development server
docker-compose -f docker-compose.dev.yml restart fermi-dev
```
...
docker build -t fermi-land:latest .

# Run production container
docker run -p 3000:3000 --env-file .env.local fermi-land:latest
```
...
docker-compose exec fermi-app node -e "
...
# Stop container
docker stop fermi-land

# Remove container
docker rm fermi-land

# Remove image
docker rmi fermi-land
```
...
docker-compose exec fermi-app npm run lint

# Scale services
docker-compose up --scale fermi-app=3
```
...
docker-compose -f docker-compose.dev.yml exec fermi-dev npm run test
...
docker-compose exec fermi-app env | grep STRIPE

# Check all environment variables in container
docker-compose exec fermi-app env

# Verify host environment variables are being passed
docker-compose exec fermi-app printenv | grep STRIPE
...
# Inspect container
docker-compose exec fermi-app sh
```
...
docker inspect fermi-land

# Check container resources
docker stats fermi-land

# View container filesystem
docker exec -it fermi-land sh

# Check network connectivity
docker network ls
docker network inspect fermi-land_fermi-network
```
  ...
  build:
    ...
    steps:
      ...
      - name: Build Docker image
        run: docker build -t fermi-land .
      - name: Run tests
        run: docker run fermi-land npm test
      - name: Deploy to production
        run: |
          docker tag fermi-land:latest your-registry/fermi-land:latest
          docker push your-registry/fermi-land:latest
```
...
# Pull from registry
docker pull your-registry/fermi-land:latest
```
...
docker-compose logs -f fermi-app

# View specific service logs
docker-compose logs -f --tail=100 fermi-app

# Export logs
docker-compose logs fermi-app > app.log
```
...
**Note**: This Docker setup is optimized for the Fermi Learning Management System. For production deployments, consider using orchestration tools like Kubernetes or Docker Swarm for better scalability and management.
